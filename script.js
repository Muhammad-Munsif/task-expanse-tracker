document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateBalance() {
        ...
        balanceElement.className = total >= 0 ? 'text-2xl font-bold text-green-600' : 'text-2xl font-bold text-red-600';
    }
            ...
            <div class="transaction-item bg-white p-4 rounded-lg border border-gray-200 flex justify-between items-center fade-in">
                <div class="flex items-center">
                    <div class="h-10 w-10 rounded-full flex items-center justify-center 
                                ${transaction.type === 'income' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'} mr-3">
                        <i class="fas ${transaction.type === 'income' ? 'fa-arrow-down' : 'fa-arrow-up'}"></i>
                    </div>
                    <div>
                        <h3 class="font-medium text-gray-800">${transaction.text}</h3>
                        <p class="text-xs text-gray-500">${formatDate(transaction.date)}</p>
                    </div>
                </div>
                <div class="flex items-center">
                    <span class="${transaction.type === 'income' ? 'text-green-600' : 'text-red-600'} font-medium mr-2">
                        ${transaction.type === 'income' ? '+' : '-'}$${Math.abs(transaction.amount).toFixed(2)}
                    </span>
                    <button onclick="deleteTransaction(${transaction.id})" class="text-gray-400 hover:text-red-600">
